...
let searchtext = docuemnt.querySelector(".#searchtext");
...
async function getweather(searchval) {
  let getdata = fetch(
    `https://api.weatherapi.com/v1/current.json?key="e174e1c22a6530182c2fd557758f090d"&q=${searchval}&aqi=no`
  );
  ...
  if ("error" in data) {
    ...
    document
      ...
      .setAttribute("src", "images//pngegg(1).png");
    celsius.innerHTML = "!";
    status1.textContent = "!";
